...
initial_id = 4871;
prn_number = [16,22,26,29,31,32];
%time = 289021.179296:0.02:289073.059296;
initial_idt = 0;
for id_data = 4871:80000
    ...
    for id_sv = 1:size(prn_number,2)
        ...
        %Prompt i/Prompt q
        newvalue3 = [TckResultCT_pos(prn_number(id_sv)).P_i(id_data).^2 / TckResultCT_pos(prn_number(id_sv)).P_q(id_data).^2];
        pioverpq = [pioverpq; newvalue3];
    ...
    %features
    training_dataset_3(initial_idt,1) = mean(abs(earlyminuslate2));
    training_dataset_3(initial_idt,2) = std(abs(earlyminuslate2));
    training_dataset_3(initial_idt,3) = mean(abs(pioverpq));
    training_dataset_3(initial_idt,4) = std(abs(pioverpq));   
    training_dataset_3(initial_idt,5) = mean(abs(navSolutionsCT.satEA(initial_idt,:)));
    training_dataset_3(initial_idt,6) = std(abs(navSolutionsCT.satEA(initial_idt,:)));
    training_dataset_3(initial_idt,7) = mean(abs(Acquired.SNR));
    training_dataset_3(initial_idt,8) = std(abs(Acquired.SNR));

    %outputs
    training_dataset_3(initial_idt,9) = sqrt(navSolutionsCT.usrPosENU(initial_idt,1).^2+navSolutionsCT.usrPosENU(initial_idt,2).^2+navSolutionsCT.usrPosENU(initial_idt,3).^2);
